...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, username, phone_number, email, password, is_staff
        ...
        now = timezone.now()
        if not username:
            raise ValueError('The given username must to be set')
    ...
    def create_user(self, username=None, phone_number=None, email=None, password=None,
                    **extra_fields):
        if username is None:
            if email:
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def is_loggedin_user(self):
        ...
        return self.phone_number is not None and self.email is not None

    #def save(self, *args, **kwargs):
       # if self.email if not None and self.email.strip() == '':
            #self.email = None
...
class Device(models.Model):
    ...
    device_type = models.PositiveIntegerField(choices=DEVICE_TYPE_CHOICES, default=WEB)
